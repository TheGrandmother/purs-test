...
testWorld1 =
  fromFoldable
    ...
    , Source (sample "test.wav" (pwmTrain 0.2 0.1) 1.0)
    ]
...
traverse l = do
  ...
  _traverse n ((Medium m0 _dx) : behind) ((Medium m1 dx) : ahead) = do
    ...
    if pick < (abs rc) then do
      ...
      pure $ Just ((transportNode m1 dx 1.0) $ rec)
    where
    rc = (z1 - z0) / (z1 + z0)
      where
      z1 = impedance m1 * dx
  ...
  _traverse _ _ (Mic : Nil) = pure Nothing
...
transportNode m x rc = \n -> case n of
  Just nn -> El.sdelay delay $ El.mul (attenuation * sign rc) $ nn
  Nothing -> El.quiet
  where
